...
Use `tr:props="${props}"` when component inputs should be serialized separately from the reactive `tr:state`; both are available to an adopted SFC, while declared object-component props are split from undeclared attributes as usual.

The SFC template compiler resolves the Vue builtin components `<transition>`, `<transition-group>`, `<keep-alive>`, `<teleport>`, and `<suspense>` intrinsically. It supports HTML, interpolation, `v-if`/`v-else-if`/`v-else`, `v-show`, `v-for` (including `(item, index) in items` and `of` syntax), `v-text`, `v-html`, `v-model`, static and dynamic `:prop`/`v-bind` arguments, static and dynamic `@event`/`v-on` arguments (including `method()` calls and `.prevent`, `.stop`, `.self`, `.once`, keyboard, mouse-button, and system modifiers), `v-on="listeners"` object bindings, registered custom directives such as `v-focus:field.trim="message"`, and default or named `<slot>` content with fallbacks. Pass directives through the render scope as `directives: { focus }`. Named slot content can use `v-slot:name`, `#name`, or `slot="name"`, including dynamic names such as `v-slot:[activeSlot]`; receiving slots can use `:name`. Receiving slots can pass static, bound, or `v-bind` props and parent templates can consume them with `#name="slotProps"` or `#name="{ label }"`. `v-html` assigns trusted HTML directly and should not receive untrusted user input. The browser registers `components/Counter.vue` against the `counter` root name, so the server's file-based HMR event remains component-scoped even when filenames and rendered component names differ.

Resource SFCs can use a CSP-safe `<script setup>` subset without evaluating arbitrary JavaScript: `ref(initial)`, `reactive(initial)`, `computed(() => expression)`, `defineOptions({ inheritAttrs: false })`, `const props = defineProps(["label"])`, `withDefaults(defineProps(["label"]), { label: "Fallback" })`, `const emit = defineEmits(["event"])`, `const model = defineModel()` (or `defineModel("title", { default: "Fallback" })`), and methods with zero or more identifier parameters composed of assignments, increments/decrements, and `emit("event", value)`. The macro arrays declare the component's props and emitted events as well as exposing the local variables; `withDefaults` and `defineModel` accept safe literal defaults and clone object/array defaults per instance, while `defineModel` declares its prop plus the matching `update:*` event. `defineExpose({ count, reset })` follows the Vue public-instance contract: a parent holding a template ref on the component reads only the declared keys (unwrapped and live), instead of the component internals. Object components get the same contract through `setup(props, { expose })`. `const slots = useSlots()` and `const attrs = useAttrs()` bind the component's live slots object and readonly attrs into the script setup scope and template expressions; object components can call the same helpers anywhere inside `setup()` and always receive the cached context. When an SFC replaces a server-rendered component, its handlers retain the Thymeleaf convention of receiving component state first, so an SFC `@click="increment"` works with `increment(state)`. Refs are automatically unwrapped in templates and `v-model`. Template-only HMR updates preserve existing script-setup local state, including components adopted from the server-rendered first paint; a changed script setup rebuilds the component so its new setup logic takes effect. Unsupported statements fail explicitly instead of being evaluated dynamically.

Resource SFCs support `<style>` and `<style scoped>` blocks. Styles are injected into `document.head` as a single `data-tr-sfc` element per component; recompiling the same source refreshes that element instead of duplicating it. Scoped blocks derive a stable `data-v-*` scope id from the SFC source, stamp it on every element the component's template renders, and rewrite each selector's last compound (including pseudo-classes, pseudo-elements, and `:deep()`) to require it, so styles never leak between components; parent-provided slot content keeps the parent's scope. Plain `<style>` blocks stay global. Inside `@media`/`@supports` the inner rules are scoped recursively; `:global(...)` escapes the scope and `@keyframes` pass through unscoped. `<style module>` blocks (with an optional injection name) rename every class with the file hash and expose the mapping to templates as `$style` or the custom name. When HMR replaces a component definition, its injected style element is removed along with the old definition.
...
The browser runtime also exports `computed(() => ...)` for cached derived state when authoring render-function components. `shallowRef()` tracks only replacement of its value and `triggerRef()` can explicitly notify dependents after a deliberate deep mutation, which is useful for large state objects. `watch` accepts a getter, ref, reactive object, or an array of sources such as `watch([userId, () => route.name], callback)`. It supports `immediate`, `once`, explicit `flush: "sync"`, `flush: "pre"`, and `flush: "post"` scheduling, plus Vue 3.6's depth-limited deep watching: `deep: true` tracks every nested level, `deep: N` tracks only N levels of nesting (ideal for large trees where full traversal is wasteful), and `deep: false`/`0` on a reactive source watches only its root properties. Queued watchers are deduplicated and run around the component render queue. `watchEffect()` accepts the same flush options, with `watchSyncEffect()` and `watchPostEffect()` as convenience APIs.
...
The VNode utility surface includes `isVNode(value)`, `cloneVNode(vnode, extraProps)`, and `mergeProps(...sources)`. `mergeProps` combines class/style values and appends event handlers in declaration order; `cloneVNode` creates a fresh patchable node while retaining its normalized children.
...
The render-function runtime exports `Suspense` for async component boundaries. Its `fallback` VNode is rendered while a descendant created by `defineAsyncComponent` is pending; once the loader resolves, the fallback is patched into the resolved content in place. Async loaders accept either a component directly or an ES module default export from `import()`. Nested boundaries isolate their pending descendants, so an inner async component shows its own fallback without replacing already-renderable outer content. `defineAsyncComponent` accepts Vue-compatible `loadingComponent`, `errorComponent`, `delay`, `timeout`, and `onError(error, retry, fail, attempts)` options, so a transient component or HMR module fetch can be retried without discarding the mounted async boundary.
